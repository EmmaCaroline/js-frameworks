...
const ProductCard = ({ product }) => {
  // Calculate the discount if there is a difference between price and discountedPrice
  ...
  return (
    <div className="flex flex-col items-center group my-4 border-gray-300">
      <div className="relative">
        ...
        />
        {/* Display the discount percentage in the top corner */}
        {discount > 0 && (
      ...
      {/* Show price and discount if applicable */}
      ...
      <div className="flex mt-2">
        {Array(fullStars)
          ...
          .map((star, index) => (
            <span key={`full-${index}`}>{star}</span> // Add a unique key for each full star
          ))}
        {halfStar === 1 && (
          <span key="half">
        ...
        {Array(emptyStars)
          ...
          .map((star, index) => (
            <span key={`empty-${index}`}>{star}</span> // Add a unique key for each empty star
          ))}
      </div>
    </div>
  );
